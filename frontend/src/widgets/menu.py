from PyQt6.QtWidgets import QMenuBar, QMenu, QWidget, QFileDialog, QMessageBox
...
class MainMenuBar(QMenuBar):
    ...
    def save_state(self):
        ...
        fileName, _ = QFileDialog.getSaveFileName(self, "Save State", "", "State Files (*.state)")
        if fileName:
            self.perform_save(fileName)  # Assuming perform_save method exists to actually save the state

    def open_state(self):
        # Opens a dialog to open a file
        fileName, _ = QFileDialog.getOpenFileName(self, "Open State", "", "State Files (*.state)")
        if fileName:
    ...
    def exit_app(self):
        ...
        reply = QMessageBox.warning(self, 'Exit', 'Application will be closed.', QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.Cancel)
